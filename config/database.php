...
define('DB_NAME', 'theultra_DB');  // Your database name
define('DB_USER', 'theultra_theultra');   // Your database username
...
// Set default timezone to Kenya (East Africa Time: UTC+3)
date_default_timezone_set('Africa/Nairobi');

// Set MySQL timezone to match
try {
...
function timeAgo($datetime) {
    try {
        ...
        $totalSeconds = 0;
        $totalSeconds += $diff->y * 365 * 24 * 60 * 60;
        $totalSeconds += $diff->m * 30 * 24 * 60 * 60;
        $totalSeconds += $diff->d * 24 * 60 * 60;
        $totalSeconds += $diff->h * 60 * 60;
        $totalSeconds += $diff->i * 60;
        $totalSeconds += $diff->s;
        ...
        if ($totalSeconds < 60) {
            ...
            return $diff->h . 'h ago';
        } elseif ($totalSeconds < 604800) { // Less than 7 days
            return $diff->d . 'd ago';
        } elseif ($totalSeconds < 2592000) { // Less than 30 days
            $weeks = floor($diff->d / 7);
            return $weeks . 'w ago';
        } elseif ($totalSeconds < 31536000) { // Less than 365 days
...
function updateSystemSetting($key, $value) {
    global $db;
    try {
        // Convert value to string for storage (system_settings stores as string)
        ...
        ");
        $result = $stmt->execute([$key, $value_str]);
        $stmt->closeCursor(); // Close cursor after update
        
        // Log the update for audit purposes
        if ($key === 'daily_withdrawal_limit') {
            error_log("DAILY WITHDRAWAL LIMIT UPDATED: New value = " . $value_str);
        }
        
        return $result;
    } catch (Exception $e) {
        error_log("Failed to update system setting '$key': " . $e->getMessage());
        return false;
    }
}
...
/**
 * Get total amount of completed admin capital injections
 * This represents capital injected by admins that contributes to platform liquidity
 * Only completed injections are counted (not pending or cancelled)
 */
function getCompletedAdminInjections($db = null) {
    global $db;
    try {
        // Only count injections that have status = 'completed' or NULL (for backward compatibility with old records)
        // Exclude pending and cancelled injections
        $stmt = $db->query("
        ...
        error_log("Failed to get completed admin injections: " . $e->getMessage());
        return 0.0;
    }
}
...
function logSystemHealth() {
    global $db;
    try {
        // Get statistics from direct table queries
        $stats = [];
        ...
        // Get total completed admin capital injections - contributes to platform liquidity
        // This ensures that any capital injected by admins automatically improves platform liquidity
        $admin_injections = getCompletedAdminInjections($db);
        
        // Calculate platform liquidity: deposits - withdrawals + admin injections
        // Admin injections represent additional capital that improves platform liquidity and system health
        $platform_liquidity = ($stats['total_deposits'] - $stats['total_withdrawals']) + $admin_injections;
        $total_liabilities = $stats['total_user_balances'] + $stats['pending_roi_obligations'] + $stats['total_active_investments'];
...
/**
 * Extract ROI amount from transaction description
 * Handles various description formats for ROI payments
 */
function extractROIFromDescription($description) {
    if (empty($description)) {
        return 0;
    }
    
    // Try different patterns to extract ROI amount
    $patterns = [
        '/ROI:\s*([0-9,]+\.?[0-9]*)/',           // "Principal: X + ROI: Y"
        '/ROI\s*([0-9,]+\.?[0-9]*)/',             // "ROI Y"
        '/\+ ROI:\s*([0-9,]+\.?[0-9]*)/',         // "Principal: X + ROI: Y"
        '/ROI:\s*KSh\s*([0-9,]+\.?[0-9]*)/',      // "ROI: KSh Y"
        '/ROI\s*KSh\s*([0-9,]+\.?[0-9]*)/',       // "ROI KSh Y"
        '/ROI\s*=\s*([0-9,]+\.?[0-9]*)/',         // "ROI = Y"
        '/ROI\s*([0-9,]+\.?[0-9]*)\s*shillings/', // "ROI Y shillings"
        '/ROI\s*([0-9,]+\.?[0-9]*)\s*KSh/',        // "ROI Y KSh"
    ];
    
    foreach ($patterns as $pattern) {
        if (preg_match($pattern, $description, $matches)) {
            $amount = (float)str_replace(',', '', $matches[1]);
            error_log("ROI extracted: $amount from description: $description");
            return $amount;
        }
    }
    
    // If no pattern matches, log for debugging
    error_log("No ROI pattern matched for description: $description");
    return 0;
}
...
// Include system helper functions (including daily withdrawal limit checking)
