...
class MpesaIntegration {
    ...
    public function __construct() {
        ...
        $this->b2c_callback_url = SITE_URL . '/api/mpesa-b2c-result.php';
    ...
    /**
 * M-Pesa B2C (Business to Customer) Payment
 * Used for instant withdrawals
 */
public function b2cPayment($phone_number, $amount, $account_reference, $remarks) {
    try {
        ...
        // B2C requires initiator credentials - you'll get these from Safaricom
        $initiator_name = getSystemSetting('mpesa_initiator_name', '');
        $security_credential = getSystemSetting('mpesa_security_credential', '');
        
        if (empty($initiator_name) || empty($security_credential)) {
            // Log missing credentials
            $this->logError("B2C credentials missing - Initiator: " . ($initiator_name ? 'Set' : 'Missing') . ", Security: " . ($security_credential ? 'Set' : 'Missing'));
            
            // For testing, return simulated success
            return [
                'success' => true,
                'ConversationID' => 'AG_' . time() . '_' . rand(1000, 9999),
                'OriginatorConversationID' => 'OC_' . time(),
                'ResponseDescription' => 'Accept the service request successfully (Simulated - B2C credentials not configured)',
                'simulated' => true
            ];
        }
        ...
        $curl_post_data = [
            ...
            'QueueTimeOutURL' => SITE_URL . '/api/mpesa-b2c-timeout.php',
            'ResultURL' => $this->b2c_callback_url,
            'Occassion' => substr($account_reference, 0, 100)
        ];
        ...
        // Log the request
        $this->logRequest('B2C_PAYMENT', $url, $curl_post_data, $response, $httpCode, $error);
        
        if ($error) {
            $this->logError("B2C Payment CURL Error: $error");
        ...
        if ($httpCode === 200 && $response) {
            ...
            $this->logRequest('B2C_RESPONSE', 'Response', $result, $response, $httpCode, null);
            
            if (isset($result['ResponseCode']) && $result['ResponseCode'] === '0') {
                $this->logSuccess("B2C Payment initiated: " . ($result['ConversationID'] ?? 'N/A'));
                return [
                    'success' => true,
                    'ConversationID' => $result['ConversationID'] ?? null,
                    'OriginatorConversationID' => $result['OriginatorConversationID'] ?? null,
                    'ResponseDescription' => $result['ResponseDescription'] ?? 'Payment initiated'
                ];
            } else {
                $error_message = $result['ResponseDescription'] ?? $result['errorMessage'] ?? 'B2C payment failed';
                $this->logError("B2C Payment failed: " . $error_message . " | Full response: " . $response);
                return ['success' => false, 'message' => $error_message];
            }
        }
        
        $this->logError("B2C Payment HTTP Error: Code $httpCode, Response: $response");
        return ['success' => false, 'message' => 'Withdrawal service temporarily unavailable. HTTP Code: ' . $httpCode];
        
    } catch (Exception $e) {
        $this->logError("B2C Payment Exception: " . $e->getMessage());
        return ['success' => false, 'message' => 'Withdrawal system error'];
    }
}
    /**
     * Get access token from M-Pesa API with retry mechanism
     */
    private function getAccessToken($retry = true) {
        ...
        // Log the request for debugging with detailed information
        $this->logRequest('GET_TOKEN', $url, [], $response, $httpCode, $error);
        
        // Check for network timeout errors (CURLE_OPERATION_TIMEDOUT = 28)
        if ($error && ($curl_errno == 28 || stripos($error, 'timeout') !== false)) {
            $this->logError("CURL Timeout getting token (Error #$curl_errno): $error");
            
            // Retry once for timeout errors only (not authentication failures)
            if ($retry) {
                $this->logError("Retrying token request after 2 seconds...");
                sleep(2);
                return $this->getAccessToken(false); // Don't retry again
            }
        ...
        if ($error) {
            $this->logError("CURL Error getting token (Error #$curl_errno): $error");
            return null;
        }
        ...
        if ($httpCode === 200 && $response) {
            ...
            if (isset($result['access_token'])) {
                ...
                $this->logError("Access token not found in response. Response: " . substr($response, 0, 500));
            }
        } else {
            $this->logError("Failed to get access token. HTTP Code: $httpCode, Response: " . substr($response, 0, 500));
        }
    ...
    /**
     * Initiate STK Push payment with retry mechanism for network errors
     */
    public function stkPush($phone_number, $amount, $account_reference, $transaction_desc, $retry = true) {
        try {
            // Validate inputs
            ...
            // Check for network timeout errors (CURLE_OPERATION_TIMEDOUT = 28)
            if ($error && ($curl_errno == 28 || stripos($error, 'timeout') !== false)) {
                $this->logError("STK Push CURL Timeout (Error #$curl_errno): $error - Phone: $phone_number, Amount: $amount");
                
                // Retry once for timeout errors only (not authentication failures)
                if ($retry) {
                    $this->logError("Retrying STK Push after 2 seconds...");
                    sleep(2);
                    return $this->stkPush($phone_number, $amount, $account_reference, $transaction_desc, false); // Don't retry again
                }
            ...
            if ($error) {
                $this->logError("STK Push CURL Error (Error #$curl_errno): $error - Phone: $phone_number, Amount: $amount");
...
private function formatPhoneNumber($phone) {
    ...
    /**
     * Log requests for debugging with detailed information
     */
    private function logRequest($type, $url, $data, $response, $httpCode, $error = null) {
        $log_data = [
            ...
            'response_preview' => substr($response ?? '', 0, 200) // First 200 chars for quick debugging
        ];
