...
try {
    // Use the fixed ROI processing script
    include __DIR__ . '/process-matured-packages.php';
    echo "✓ ROI processing completed\n";
} catch (Exception $e) {
...
try {
    // Include the B2C result handler to use its auto-check function
    require_once __DIR__ . '/../api/mpesa-b2c-result.php';
    
    // Call the automatic withdrawal checker
    checkAndUpdatePendingWithdrawals();
    
    // Also process any withdrawals that have been pending for too long
    $stmt = $db->query("
    ...
    foreach ($old_withdrawals as $withdrawal) {
        try {
            ...
            // AUTOMATED: Mark as completed (assuming successful after 2 hours timeout)
            $stmt = $db->prepare("
                UPDATE transactions 
                SET status = 'completed',
                    mpesa_receipt = ?,
                    description = CONCAT(COALESCE(description, ''), ' - Automated: Auto-completed by cron after 2 hours timeout'),
                    updated_at = NOW()
                WHERE id = ?
            ");
            $auto_receipt = 'CRON' . time() . rand(1000, 9999);
    ...
    echo "✓ Processed {$processed} old withdrawals\n";
