...
foreach ($pending_withdrawals as $withdrawal) {
    try {
        ...
        if ($result['success']) {
            // AUTOMATED: Update transaction status
            // Note: With new automated system, withdrawals are processed immediately
            // This cron is for legacy scheduled withdrawals
            $stmt = $db->prepare("
                UPDATE transactions 
                SET status = 'processing',
                    mpesa_request_id = ?,
                    description = CONCAT(description, ' - Automated: Processed by scheduled cron'),
                    updated_at = NOW()
                WHERE id = ?
            ");
            ...
            $stmt = $db->prepare("
                UPDATE transactions 
                SET status = 'failed',
                    description = CONCAT(description, ' - Automated: Scheduled cron process failed - ', ?),
                    updated_at = NOW()
                WHERE id = ?
            ");
