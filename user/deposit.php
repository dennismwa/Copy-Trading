...
$minimum_deposit = (float)getSystemSetting('minimum_deposit_amount', 100);
...
if ($_POST && isset($_POST['make_deposit'])) {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        // Remove any non-numeric characters from phone (handles spaces, dashes, parentheses, etc.)
        $phone = preg_replace('/[^0-9+]/', '', $phone);
        
        // Remove + if present
        $phone = str_replace('+', '', $phone);
        
        // Remove any non-numeric characters (final cleanup)
        $phone = preg_replace('/[^0-9]/', '', $phone);
        ...
        if (strlen($phone) == 10 && substr($phone, 0, 1) === '0') {
            ...
            // Handle case where there's an extra digit
            $phone = substr($phone, 0, 12);
        }
        
        // Validation
        if ($amount < $minimum_deposit) {
            $error = 'Minimum deposit amount is KSh ' . number_format($minimum_deposit, 0) . '.';
            ...
            $digit4 = substr($phone, 3, 1);
            $digit5 = substr($phone, 4, 1);
            ...
            if (!$is_valid) {
                ...
                try {
                    ...
                    if ($mpesa_result['success']) {
                        ...
                        // Rollback transaction to prevent orphaned pending transactions
                        $db->rollBack();
                        
                        // Provide user-friendly error messages
                        $error_message = $mpesa_result['message'] ?? 'Failed to initiate M-Pesa payment. Please try again.';
                        
                        // Map technical errors to user-friendly messages
                        if (stripos($error_message, 'timeout') !== false || stripos($error_message, 'Connection timed out') !== false) {
                            $error = 'Unable to connect to M-Pesa service. Please check your internet connection and try again in a few moments.';
                        } elseif (stripos($error_message, 'authenticate') !== false || stripos($error_message, 'authentication') !== false) {
                            $error = 'M-Pesa service is temporarily unavailable. Please try again in a few minutes or contact support if the issue persists.';
                        } elseif (stripos($error_message, 'network') !== false || stripos($error_message, 'connection') !== false) {
                            $error = 'Network error occurred. Please check your internet connection and try again.';
                        } elseif (stripos($error_message, 'configured') !== false || stripos($error_message, 'configuration') !== false) {
                            $error = 'Payment service is currently being configured. Please try again later or contact support.';
                        } else {
                            $error = $error_message;
                        }
                        
                        error_log("M-Pesa STK Push Failed - User: $user_id, Amount: $amount, Phone: $phone, Error: " . ($mpesa_result['message'] ?? 'Unknown error') . ", Transaction ID: $transaction_id (rolled back)");
                    }
                    ...
                    <form method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 md:grid-cols-4 gap-3 mb-4">
                                ...
                                // Filter quick amounts to only show those >= minimum deposit
                                $quick_amounts = array_values(array_filter($quick_amounts, function($amt) use ($minimum_deposit) {
                                    return $amt >= $minimum_deposit;
                                }));
                                // If no quick amounts meet minimum, add the minimum itself
                                if (empty($quick_amounts)) {
                                    $quick_amounts = [$minimum_deposit];
                                }
                        ...
                        <div>
                            ...
                            <div class="flex justify-between text-sm text-gray-500 mt-2">
                                <span>Minimum: KSh <?php echo number_format($minimum_deposit, 0); ?></span>
...
<?php include __DIR__ . '/../chat/widget/chat-widget-loader.php'; ?>
    ...
    <script>
        ...
        const minimumDeposit = <?php echo $minimum_deposit; ?>;
        ...
        // Form submission with validation and protection
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            // Validate amount
            if (!amount || amount < minimumDeposit) {
                validationError = 'Minimum deposit amount is KSh ' + minimumDeposit.toLocaleString() + '.';
            } else if (amount > 1000000) {
                validationError = 'Maximum deposit amount is KSh 1,000,000.';
            }
            
            // Validate phone number
            if (!phone) {
                validationError = 'Please enter your M-Pesa phone number.';
            } else {
                // Remove non-numeric characters for validation
                const phoneDigits = phone.replace(/\D/g, '');
                if (phoneDigits.length < 9 || phoneDigits.length > 13) {
                    validationError = 'Please enter a valid M-Pesa phone number (e.g., 0712345678 or 254712345678).';
                }
            }
            ...
            if (validationError) {
                ...
                submitBtn.disabled = false;
                submitBtn.innerHTML = '<i class="fas fa-mobile-alt mr-2"></i>Send M-Pesa Request';
                return false;
            }
            ...
            // Re-enable button after 10 seconds as a safety measure (in case of network issues)
            setTimeout(function() {
                if (formSubmitting) {
