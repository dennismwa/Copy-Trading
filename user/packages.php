    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (!$package) {
            ...
            try {
                ...
                // ============================================================
                // STRICT DAILY PACKAGE TRADE LIMIT ENFORCEMENT
                // ============================================================
                // Use the centralized function for consistent limit checking
                // This function handles locking, limit retrieval, and validation
                $trade_limit_check = checkDailyPackageTradeLimit($db, $user_id, $package_id, true);
                
                if (!$trade_limit_check['allowed']) {
                    $db->rollBack();
                    $error = $trade_limit_check['message'];
                    throw new Exception($error);
                }
                
                // Daily trade limit check passed - proceed with package activation
                // CORRECTED: Deduct investment amount from wallet balance
                // When user invests, the money moves from wallet to active investment
                $stmt = $db->prepare("
                    UPDATE users 
                ...
                // Create the investment transaction record (for tracking purposes)
                $stmt = $db->prepare("
                ...
                // Create active package record with explicit status='active' for immediate system health visibility
                $stmt = $db->prepare("
                    INSERT INTO active_packages (user_id, package_id, investment_amount, expected_roi, roi_percentage, duration_hours, maturity_date, status) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, 'active')
                ");
