...
// Handle view parameter (when clicking notification from dropdown)
$view_notification_id = isset($_GET['view']) ? (int)$_GET['view'] : null;
if ($view_notification_id) {
    try {
        // Mark the notification as read if it's being viewed
        // Try with updated_at first, fallback if column doesn't exist
        try {
            $stmt = $db->prepare("
                UPDATE notifications 
                SET is_read = 1, updated_at = NOW() 
                WHERE id = ? AND (user_id = ? OR is_global = 1) AND is_read = 0
            ");
    ...
    // Switch to 'all' filter to find the notification
    $filter = 'all';
}
        ...
        <section class="glass-card rounded-xl p-6">
            ...
            <?php elseif ($error_message): ?>
                <div class="p-8 text-center">
                    ...
                    <p class="text-red-400 mb-2">Error loading notifications</p>
                    <p class="text-gray-400 text-sm"><?php echo htmlspecialchars($error_message); ?></p>
                    <a href="/user/notifications.php" class="mt-4 inline-block px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition">
                        Try Again
                    </a>
                </div>
            <?php else: ?>
                <div class="space-y-4">
                    ...
                    <div class="notification-item p-4 rounded-lg <?php echo $notification['is_read'] ? 'bg-gray-800/30' : 'unread bg-gray-800/50'; ?> <?php echo ($view_notification_id && $view_notification_id == $notification['id']) ? 'ring-2 ring-emerald-500' : ''; ?>" data-notification-id="<?php echo $notification['id']; ?>" id="notification-<?php echo $notification['id']; ?>">
...
<?php include __DIR__ . '/../chat/widget/chat-widget-loader.php'; ?>
    ...
    <script>
        ...
        // Scroll to notification if view parameter is set
        <?php if ($view_notification_id): ?>
        window.addEventListener('load', function() {
            const notificationElement = document.getElementById('notification-<?php echo $view_notification_id; ?>');
            if (notificationElement) {
                setTimeout(() => {
                    notificationElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    // Highlight it briefly
                    notificationElement.classList.add('ring-2', 'ring-emerald-500');
                    setTimeout(() => {
                        notificationElement.classList.remove('ring-2', 'ring-emerald-500');
                    }, 3000);
                }, 500);
            }
        });
