    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($subject) || empty($message)) {
            ...
            try {
                ...
                $stmt = $db->prepare("
                    INSERT INTO support_tickets (ticket_number, user_id, subject, message, priority, category, status) 
                    VALUES (?, ?, ?, ?, ?, ?, 'open')
                ");
...
$total_tickets = (int)($count_result['total'] ?? 0);
$total_pages = ceil($total_tickets / $limit);

$stmt = $db->prepare("
    SELECT st.*, 
           CASE 
               WHEN st.admin_id IS NOT NULL THEN admin_user.full_name 
               ELSE NULL 
           END as admin_name
...
foreach ($tickets as &$ticket) {
    ...
    ");
    $response_stmt->execute([$ticket['id']]);
    $ticket['responses'] = $response_stmt->fetchAll(PDO::FETCH_ASSOC);
    $response_stmt->closeCursor();
}
...
// Ensure stats array has all required keys with default values
if (!$stats) {
    $stats = [
        'total_tickets' => 0,
        'open_tickets' => 0,
            ...
            <div class="lg:col-span-1">
                <div class="glass-card rounded-xl p-6">
                    ...
                    <nav class="space-y-2">
                        ...
                        <a href="mailto:support@theultraharvest.com" class="nav-item w-full text-left px-4 py-3 rounded-lg transition hover:bg-gray-700 block">
                ...
                <div class="glass-card rounded-xl p-6 mt-6">
                    ...
                    <a href="mailto:support@theultraharvest.com" class="block w-full px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition text-center">
                        <i class="fas fa-envelope mr-2"></i>support@theultraharvest.com
                    </a>
                                                    ...
                                                    <div class="bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-4">
                                                        <div class="flex items-center mb-2">
                                                            ...
                                                            <span class="font-medium text-emerald-300">
                                                                Response from <?php echo htmlspecialchars($response['full_name'] ?? $ticket['admin_name'] ?? 'Support Team'); ?>
                                                            </span>
                                                            <span class="ml-2 px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded text-xs font-medium">Admin</span>
                                                        </div>
                                                        <p class="text-gray-300 whitespace-pre-wrap"><?php echo htmlspecialchars($response['response_text']); ?></p>
...
<br><br>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-gray-800 border-t border-gray-700 md:hidden z-40">
        <div class="grid grid-cols-5 py-2">
            ...
            <a href="/user/socials.php" class="flex flex-col items-center py-2 text-blue-400">
...
<?php include __DIR__ . '/../chat/widget/chat-widget-loader.php'; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Check if we should show tickets section (if URL has #tickets or ?section=tickets)
            const urlParams = new URLSearchParams(window.location.search);
            const hash = window.location.hash;
            if (urlParams.get('section') === 'tickets' || hash === '#tickets') {
                showSection('tickets');
            } else {
                showSection('create');
            }
