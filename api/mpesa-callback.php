...
try {
    ...
    if ($result_code == '0') {
        ...
        if ($transaction['type'] === 'deposit') {
            ...
            // CRITICAL FIX: Process referral commissions (only for first deposit)
...
function processReferralCommissions($transaction, $db) {
    try {
        ...
        // NEW RULE: Check if this is the user's first deposit
        // Only pay commission on FIRST deposit by referred user
        $stmt = $db->prepare("
        ...
        $deposit_count = (int)$deposit_data['deposit_count'];
        
        // If this is not the first deposit, skip commission processing
        if ($deposit_count > 1) {
            error_log("SKIPPING COMMISSION: User {$transaction['user_id']} has made {$deposit_count} deposits. Commission only paid on first deposit.");
            
            // Audit log for skipped commission
            error_log(sprintf(
                "REFERRAL COMMISSION SKIPPED: User ID %d made deposit #%d of %s. Referrer ID: %d would have earned commission, but rule changed to only pay on first deposit.",
                $transaction['user_id'],
                $deposit_count,
        ...
        // This is the first deposit - proceed with commission
        error_log("FIRST DEPOSIT DETECTED: User {$transaction['user_id']} made their first deposit of " . formatMoney($deposit_amount) . " - Processing commissions for referrer ID: $referrer_id");
        
        error_log("Processing commissions - User ID: {$transaction['user_id']}, Referrer ID: $referrer_id, Deposit: $deposit_amount, Deposit Count: $deposit_count");
        ...
        if ($l1_commission > 0) {
            ...
            if ($l1_referrer_data && $l1_referrer_data['referred_by']) {
                ...
                if ($l2_commission > 0) {
                    ...
                    // Automatically assign referral tier based on updated earnings
                    assignReferralTierAutomatically($db, $l2_referrer_id);
