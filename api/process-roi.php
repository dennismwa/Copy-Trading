...
// DISABLED: This script conflicts with process-matured-packages.php
// Use cron/process-matured-packages.php instead for consistent processing
echo "This ROI processing script has been disabled to prevent conflicts.\n";
echo "Please use cron/process-matured-packages.php for ROI processing.\n";
exit;
...
try {
    ...
    foreach ($matured_packages as $package) {
        ...
        try {
            ...
            echo "Processing package ID {$package_id} for user {$package['full_name']} - Investment: " . formatMoney($investment_amount) . ", ROI: " . formatMoney($roi_amount) . ", Total Return: " . formatMoney($total_return) . "\n";

            // CORRECTED: Credit both the original investment (principal) + ROI to wallet
            // Since investment was deducted at purchase, we return principal + earnings
            $stmt = $db->prepare("
                UPDATE users 
            ...
            echo "  ✓ Wallet credited with " . formatMoney($total_return) . " (Principal + ROI)\n";

            // Create ROI + Principal return transaction (record full return amount)
            $stmt = $db->prepare("
                INSERT INTO transactions (user_id, type, amount, status, description, created_at) 
                VALUES (?, 'package_completion', ?, 'completed', ?, NOW())
            ");
            $description = "Package completion for {$package['package_name']} - Principal: " . formatMoney($investment_amount) . " + ROI: " . formatMoney($roi_amount) . " ({$package['roi_percentage']}% of " . formatMoney($investment_amount) . ")";
            $result = $stmt->execute([$user_id, $total_return, $description]);
            
            if (!$result) {
                throw new Exception("Failed to create package completion transaction");
            }
            
            echo "  ✓ Package completion transaction created\n";
            ...
            sendNotification(
                $user_id,
                'Package Completed! 🎉',
                "Your {$package['package_name']} package has matured successfully! Principal " . formatMoney($investment_amount) . " + ROI " . formatMoney($roi_amount) . " has been credited to your wallet. Start a new investment to continue earning!",
                'success'
            );
    ...
    echo "Investments Returned: " . formatMoney($total_investment_returned) . "\n";
    echo "Total Amount Credited to Wallets: " . formatMoney($total_roi_paid + $total_investment_returned) . "\n";
    ...
    if ($processed_count > 0 || $failed_count > 0) {
        try {
            ...
            $summary_message .= "💼 Investments Returned: " . formatMoney($total_investment_returned) . "\n";
...
function processReferralCommissionsForROI($user_id, $referrer_id, $roi_amount, $user_name, $db) {
    try {
        ...
        if ($l1_commission > 0) {
            ...
            // Automatically assign referral tier based on updated earnings
            assignReferralTierAutomatically($db, $referrer_id);
            echo "    ✓ Tier assignment checked for L1 referrer\n";
            ...
            if ($l2_referrer && $l2_referrer['referred_by']) {
                ...
                if ($l2_commission > 0) {
                    ...
                    // Automatically assign referral tier based on updated earnings
                    assignReferralTierAutomatically($db, $l2_referrer['referred_by']);
                    echo "    ✓ Tier assignment checked for L2 referrer\n";
