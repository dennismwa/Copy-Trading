        ...
        switch ($action) {
            case 'update_status':
                ...
                try {
                    ...
                    if (!headers_sent()) {
                        header("Location: tickets.php?success=updated&ticket_id=" . $ticket_id);
                        exit;
                    } else {
                        $success = 'Ticket updated successfully.';
                    }
            ...
            case 'add_response':
                ...
                try {
                    ...
                    // POST-Redirect-GET to prevent duplicate submissions
                    if (!headers_sent()) {
                        header("Location: tickets.php?success=response_added&ticket_id=" . $ticket_id);
                        exit;
                    } else {
                        $success = 'Response added successfully.';
                    }
            ...
            case 'assign_ticket':
                ...
                try {
                    ...
                    // POST-Redirect-GET to prevent duplicate submissions
                    if (!headers_sent()) {
                        header("Location: tickets.php?success=assigned&ticket_id=" . $ticket_id);
                        exit;
                    } else {
                        $success = 'Ticket assigned successfully.';
                    }
...
try {
    ...
    error_log("Admin tickets page error: " . $e->getMessage());
    $tickets = [];
}
...
try {
    ...
    error_log("Admin tickets stats error: " . $e->getMessage());
    // Use default stats
}
...
try {
    ...
    error_log("Admin tickets - fetch admins error: " . $e->getMessage());
    $admins = [];
}
...
try {
    ...
    error_log("Admin tickets - fetch users error: " . $e->getMessage());
    $users = [];
}
?>
...
<header class="bg-gray-800/50 backdrop-blur-md border-b border-gray-700 sticky top-0 z-50">
    ...
    <div id="mobileMenu" class="hidden lg:hidden border-t border-gray-700 bg-gray-800/95 backdrop-blur-md">
        <nav class="container mx-auto px-4 py-4 space-y-2">
            ...
            <a href="/admin/system-health.php" class="block px-4 py-3 text-gray-300 hover:text-emerald-400 hover:bg-gray-700/50 rounded-lg transition">
                <i class="fas fa-headset mr-3"></i>Health
            </a>
