<?php
/**
 * Extend Package Maturity for User 274 (Raphael Kabugua)
...
try {
    ...
    // Get ALL active packages for this user (EXCLUDING packages 1528 and 1527)
    $stmt = $db->prepare("
        SELECT 
            ap.id as package_id,
            ap.maturity_date,
    ...
    $excluded_packages = $stmt_excluded->fetchAll(PDO::FETCH_ASSOC);
    ...
    echo "Active Packages to Extend: " . count($packages) . "\n";
    
    if (!empty($excluded_packages)) {
        echo "Excluded Packages (will NOT be extended): " . count($excluded_packages) . "\n";
        foreach ($excluded_packages as $excl) {
            echo "  - Package: {$excl['package_name']} (ID: {$excl['package_id']})\n";
        }
    }
    echo "\n";
